...
describe('resolveDid', () => {
    it('should resolve TATA Motors DID', () => {
        const result = (0, did_resolver_1.resolveDid)('did:eu-dataspace:company-tata-001');
    ...
    it('should resolve Digit Insurance DID', () => {
        const result = (0, did_resolver_1.resolveDid)('did:eu-dataspace:company-digit-001');
        expect(result.didDocument).not.toBeNull();
        expect(result.didDocument.id).toBe('did:eu-dataspace:company-digit-001');
    });
    it('should resolve user DIDs', () => {
        const result = (0, did_resolver_1.resolveDid)('did:smartsense:mario-sanchez');
        expect(result.didDocument).not.toBeNull();
        expect(result.didDocument.id).toBe('did:smartsense:mario-sanchez');
    });
    it('should return notFound for unknown DIDs', () => {
        const result = (0, did_resolver_1.resolveDid)('did:unknown:xyz');
    ...
    it('should include DataService and VehicleRegistryService for TATA Motors', () => {
        ...
        expect(serviceTypes).toContain(did_resolver_1.SERVICE_TYPES.VEHICLE_REGISTRY);
        expect(serviceTypes).toContain(did_resolver_1.SERVICE_TYPES.VP_VERIFICATION);
        expect(serviceTypes).toContain(did_resolver_1.SERVICE_TYPES.DATA_SERVICE);
    });
    it('should NOT include removed direct-fetch service types', () => {
        const result = (0, did_resolver_1.resolveDid)('did:eu-dataspace:company-tata-001');
        const serviceTypes = (result.didDocument.service || []).map(s => s.type);
        expect(serviceTypes).not.toContain(did_resolver_1.SERVICE_TYPES.VEHICLE_INSURANCE_DATA);
        expect(serviceTypes).not.toContain(did_resolver_1.SERVICE_TYPES.VEHICLE_DPP);
        expect(serviceTypes).not.toContain(did_resolver_1.SERVICE_TYPES.VEHICLE_CREDENTIALS);
    });
...
describe('selectEndpoint', () => {
    it('should find DataService endpoint', () => {
        ...
        const endpoint = (0, did_resolver_1.selectEndpoint)(didDocument, did_resolver_1.SERVICE_TYPES.DATA_SERVICE);
    ...
    it('should find VehicleRegistryService endpoint', () => {
        const { didDocument } = (0, did_resolver_1.resolveDid)('did:eu-dataspace:company-tata-001');
        const endpoint = (0, did_resolver_1.selectEndpoint)(didDocument, did_resolver_1.SERVICE_TYPES.VEHICLE_REGISTRY);
        expect(endpoint).not.toBeNull();
        expect(endpoint.serviceEndpoint).toContain('/vehicle-registry');
    });
    it('should return null for unknown service type', () => {
        const { didDocument } = (0, did_resolver_1.resolveDid)('did:eu-dataspace:company-tata-001');
        const endpoint = (0, did_resolver_1.selectEndpoint)(didDocument, 'UnknownServiceType');
        expect(endpoint).toBeNull();
    });
    it('should return null for removed VehicleInsuranceDataService', () => {
        const { didDocument } = (0, did_resolver_1.resolveDid)('did:eu-dataspace:company-tata-001');
        const endpoint = (0, did_resolver_1.selectEndpoint)(didDocument, did_resolver_1.SERVICE_TYPES.VEHICLE_INSURANCE_DATA);
        expect(endpoint).toBeNull();
    });
});
describe('getServiceEndpoints', () => {
    it('should return all service endpoints', () => {
        const { didDocument } = (0, did_resolver_1.resolveDid)('did:eu-dataspace:company-tata-001');
        const endpoints = (0, did_resolver_1.getServiceEndpoints)(didDocument);
