...
router.get('/credentials/:userId/ownership', async (req, res) => {
    ...
    const ownershipCreds = wallet.credentials
        ...
        .filter((c) => c && c.type === 'OwnershipVC')
        .map((c) => formatAsW3CVC(c));
...
router.post('/generate-vp', async (req, res) => {
    ...
    for (const credId of credentialIds) {
        const cred = await db_1.default.credential.findUnique({ where: { id: credId } });
        if (!cred) {
    ...
    // Generate VP
    const vp = (0, vp_processor_1.createVerifiablePresentation)(holderDid, credentials, {
        challenge,
        domain,
    });
...
function formatAsW3CVC(dbCred) {
    ...
    const vcJwt = (0, vp_processor_1.signVC)(vcPayload, resolvedIssuerDid);
    return {
        ...vcPayload,
        proof: {
