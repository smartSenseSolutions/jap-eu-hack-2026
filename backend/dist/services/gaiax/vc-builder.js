...
exports.getVCBaseUrl = getVCBaseUrl;
exports.buildLegalParticipantVC = buildLegalParticipantVC;
exports.buildTermsAndConditionsVC = buildTermsAndConditionsVC;
exports.buildRegistrationNumberVC = buildRegistrationNumberVC;
exports.validateOrgCredentialFields = validateOrgCredentialFields;
...
function buildLegalParticipantVC(org, did) {
 ...
 */
function buildTermsAndConditionsVC(did, orgId) {
    const baseUrl = getVCBaseUrl();
    return {
        '@context': [
            'https://www.w3.org/ns/credentials/v2',
 ...
 */
function buildRegistrationNumberVC(did, orgId, regNumber, countryCode) {
    const baseUrl = getVCBaseUrl();
...
function validateOrgCredentialFields(data) {
    const errors = [];
    ...
    if (!addr?.countryCode)
        errors.push('legalAddress.countryCode is required');
    if (!addr?.streetAddress)
        errors.push('legalAddress.streetAddress is required');
    if (!addr?.locality)
        errors.push('legalAddress.locality is required');
    if (!addr?.postalCode)
        errors.push('legalAddress.postalCode is required');
    return errors;
}
