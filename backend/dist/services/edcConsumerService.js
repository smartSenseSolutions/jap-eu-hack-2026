...
exports.waitForAgreement = waitForAgreement;
exports.initiateTransfer = initiateTransfer;
exports.getTransferProcess = getTransferProcess;
exports.getAuthCode = getAuthCode;
exports.fetchAssetData = fetchAssetData;
exports.negotiateAndFetchData = negotiateAndFetchData;
...
async function queryCatalog(vin, provider) {
    ...
    const response = await axios_1.default.post(`${EDC_MGMT_URL}/v3/catalog/request`, payload, { headers, timeout: 15000 });
    ...
    const match = datasetList.find((ds) => ds['@id'] === assetKey || ds.id === assetKey);
    if (!match) {
...
async function initiateNegotiation(offerId, assetId, provider) {
    const payload = {
        '@context': {
            '@vocab': 'https://w3id.org/edc/v0.0.1/ns/',
        },
    ...
    const response = await axios_1.default.post(`${EDC_MGMT_URL}/v3/contractnegotiations`, payload, { headers, timeout: 10000 });
    return response.data['@id'];
}
// Step 3: Poll for agreement until FINALIZED
async function waitForAgreement(negotiationId) {
    await sleep(NEGOTIATION_INITIAL_DELAY);
    for (let attempt = 1; attempt <= NEGOTIATION_MAX_RETRIES; attempt++) {
        const response = await axios_1.default.get(`${EDC_MGMT_URL}/v3/contractnegotiations/${negotiationId}`, {
...
async function initiateTransfer(assetId, contractAgreementId, provider) {
    const payload = {
        '@context': {
            '@vocab': 'https://w3id.org/edc/v0.0.1/ns/',
        },
    ...
    const response = await axios_1.default.post(`${EDC_MGMT_URL}/v3/transferprocesses`, payload, { headers, timeout: 10000 });
    return response.data['@id'];
}
// Step 5: Get transfer process (EDR entry) — polls with retries
async function getTransferProcess(contractAgreementId) {
    const payload = {
        '@context': {
            '@vocab': 'https://w3id.org/edc/v0.0.1/ns/',
        },
    ...
    for (let attempt = 1; attempt <= NEGOTIATION_MAX_RETRIES; attempt++) {
        ...
        const response = await axios_1.default.post(`${EDC_MGMT_URL}/v3/edrs/request`, payload, { headers, timeout: 10000 });
...
async function getAuthCode(transferId) {
    const response = await axios_1.default.get(`${EDC_MGMT_URL}/v2/edrs/${transferId}/dataaddress?auto_refresh=true`, { headers: { 'x-api-key': EDC_API_KEY }, timeout: 10000 });
...
async function fetchAssetData(endpoint, authorization) {
...
async function negotiateAndFetchData(vin, provider, onProgress, meta) {
    ...
    await db_1.default.edcTransaction.create({ data: tx });
    const emitStep = async (step, status, durationMs, details) => {
        const update = { step, totalSteps: 7, name: STEP_NAMES[step - 1], status, durationMs, details };
        if (onProgress)
            onProgress(update);
        ...
        await db_1.default.edcTransaction.update({ where: { id: txId }, data: tx });
    };
